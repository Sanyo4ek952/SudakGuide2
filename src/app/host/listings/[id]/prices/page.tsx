...
export default async function HostListingPricesPage({ params }: { params: Promise<{ id: string }> }) {
            ...
            <div key={photo.id} className="rounded border p-2">
              ...
              <div className="mt-2 flex gap-2">
                <form action={listingFeature.movePhotoAction}>
                  ...
                  <button className="rounded bg-slate-700 px-3 py-1 text-white" type="submit">↑</button>
                </form>
                <form action={listingFeature.movePhotoAction}>
                  <input type="hidden" name="photoId" value={photo.id} />
                ...
                <form action={listingFeature.removePhotoAction}>
                  <input type="hidden" name="photoId" value={photo.id} />
                  <button className="rounded bg-rose-700 px-3 py-1 text-white" type="submit">Удалить</button>
                </form>
              </div>
            </div>
          ))}
